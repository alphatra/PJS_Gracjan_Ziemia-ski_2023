...
class PhantomVoteSupabaseUser extends BaseAuthUser {
  PhantomVoteSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> phantomVoteSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = PhantomVoteSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
